...
class PropertiesTest extends TestCase
{
    ...
    public function test_property_owner_can_add_photo_to_property()
    {
        ...
        $response->assertJsonFragment([
            'filename' => config('app.url') . '/storage/' . $property->getFirstMedia('photos')->id . '/photo.png',
            'thumbnail' => config('app.url') . '/storage/' . $property->getFirstMedia('photos')->id . '/conversions/photo-thumbnail.jpg',
        ]);
    }
    ...
    public function test_property_owner_can_reorder_photos_in_property()
    {
        ...
        $response = $this->actingAs($owner)->postJson('/api/v2/owner/properties/' . $property->id . '/photos/' . $property->getFirstMedia('images')->id . '/reorder/' . $newPosition);
        $response->assertStatus(200);
